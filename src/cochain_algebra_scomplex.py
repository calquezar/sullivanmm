...
class CochainAlgebraComplex(Parent, UniqueRepresentation, CachedRepresentation):
    ...
    def _face_data_SComplex(self, dim_from, dim_to):
        K = self.K
        ...
        for face in faces:
            d = dim_to
            m = identity_matrix(QQ, len(K.n_cells(d)), sparse=True)
            for f in reversed(face):
                m = (self._face_matrix_SComplex(f, d) * m)
                d -= 1
            data[face] = m
        return data

    def _face_matrix_SComplex(self, face, dim):
        K1 = self.K._n_cells_sorted((dim - 1))
        K2 = self.K._n_cells_sorted(dim)
        M = matrix(QQ, len(K1), len(K2), (lambda i, j: (1 if (
    ...
    def _get_cohomology_dimensions(self):
        ...
        return vector(QQ, [co[d].rank() for d in range((dim + 1))], sparse=True)
    ...
    def cochain_wedge_product(self, c1, dim1, c2, dim2):
        dim = (dim1 + dim2)
        face_data1 = self._face_data_SComplex(dim1, dim)
        face_data2 = self._face_data_SComplex(dim2, dim)
        w = vector(QQ, len(self.K.n_cells(dim)), sparse=True)
    ...
    def constant(self, c):
        ...
        return self.element_class()(self, components, 0)

    def dimension(self):
        return self._dimension

    def element_class(self):
        return CochainAlgebraElementComplex
                ...
                for g in M.cohomology(degree).basis().keys():
                    ...
                    if (w == 0):
                        ...
                        if ((h == 0) and (len(h) < ncoords)):
                            h = zero_vector(QQ, ncoords)
                    imagesphico += [h]
            phico = matrix(QQ, CM.dimension(), list(imagesphico), sparse=True)
                    ...
                    for g in M.cohomology(degree).basis().keys():
                        ...
                        if (w == 0):
                            ...
                            if ((h == 0) and (len(h) < ncoords)):
                                h = zero_vector(QQ, ncoords)
                        imagesphico += [h]
                phico = matrix(QQ, CM.dimension(), list(
                    imagesphico), sparse=True).T
    ...
    def one(self):
        ...
        for dim in range((self._dimension + 1)):
            ncells = len(self.K._n_cells_sorted(dim))
            components[dim] = vector(
                self.base_ring(), (ncells * [1]), sparse=True)
        return self.element_class()(self, components, 0)
...
class CochainAlgebraElementComplex(Element):
    ...
    def differential(self):
        ...
        if self.is_homogeneous():
            if (self.degree() == C.dimension()):
                ...
                components[deg] = zero_vector(QQ, len(v))
        else:
    ...
    def get_preimage_exact_component(self):
        ...
        if (e == 0):
            return zero_vector(QQ, len(self._components[(deg - 1)]))
        else:
            hodge = C.hodge()
